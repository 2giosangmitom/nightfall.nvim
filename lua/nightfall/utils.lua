...
---@param c string: The hexadecimal color code (#RRGGBB).
...
local function hexToRgb(c)
  ...
  if not c:match("^#[0-9a-f]+$") or #c ~= 7 then error("Invalid hex color format: " .. c) end

  -- Extract R, G, B components and convert them to decimal
...
function M.blend(foreground, background, alpha)
  -- Convert hexadecimal alpha to decimal if it's in string format
  alpha = type(alpha) == "string" and (tonumber(alpha, 16) / 0xff) or alpha

  -- Safely handle errors from hexToRgb
...
function M.darken(hex, amount, bg)
  -- If bg is not provided, use default background color M.bg
...
function M.lighten(hex, amount, fg)
  -- If fg is not provided, use default foreground color M.fg
...
function M.vary_color(palettes, default)
  local flavor = require("nightfall").Options.flavor

  -- Check if the flavor option exists in the palettes table
  if palettes[flavor] ~= nil then return palettes[flavor] end

  -- Return the default color palette if the flavor option is not found
  return default
end
